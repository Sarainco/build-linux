...
static int __init mykey_init(void)
{
    ...
    if(keydev.major) {
        keydev.devid = MDEV(keydev.major, 0);
        register_chrdev_region(keydev.devid, KEY_CNT, KEY_NAME);
    } else {
        alloc_chrdev_region(&keydev.devid, 0, KEY_CNT, KEY_NAME);
        keydev.major = MAJOR(keydev.major);
        keydev.minor = MINOR(keydev.minor);
    }
...
static void __exit mykey_exit(void)
{
    ...
    device_destory(keydev.class, keydev.devid);
    class_destory(keydev.class);
}
